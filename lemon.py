...
# -*- coding: utf-8 -*-
"""
Created on Fri May 17 13:40:00 2019
...
@author: georgiabaltsou
"""
#new_graph_size = 1000 instead of 3000 which is the default
...
def lemon(file, s, myFile):
    
#    s = '283'
    
    
    with open('seedFile.txt', 'w') as seedFile:
        writer = csv.writer(seedFile)
        row = [s]
        writer.writerow(row)
        
#    file = "experiments/datasets/weighted/lfrEdgelistN1000MU0.1*.csv<61-253>6.csv"
#    
#    myFile = file.split("*")[0]
    ...
    parser.add_option("-f", "--input network file", dest="network_file", default=file,
                      help="input file of edge list for clustering [default: example_graphs/amazon/graph]")
    #give the ground truth community file
#    parser.add_option("-g", "--input community ground truth file", dest="groundtruth_community_file", default="../../experiments/datasets/lfr/lfrCommN1000MU0.1*.txt",
#                      help="input file of ground truth community membership [default: example_graphs/amazon/community]")
    ...
    parser.add_option("--sd", "--input seed set file", dest="seed_set_file", default="seedFile.txt",
...
#    test_comm = np.array([768, 513, 827, 899, 133, 71, 336, 789, 405, 278, 793, 538, 25, 156, 924, 671, 32, 609, 420, 813, 558, 434, 435])
...
#    os.chdir('experiments/datasets/communities')
    
    # write out result
    with open('communities/lemon_communities'+str(myFile)+'.csv', 'a') as out:
        writer = csv.writer(out, delimiter=';')
        
        if os.stat('communities/lemon_communities'+str(myFile)+'.csv').st_size == 0:
            writer.writerow(["Node 1", "Node 2", "Multiplied Weight", "Seed node", "Community"])
        
        row = [node1]+[node2]+[wName]+[s]+list(detected_comm)
